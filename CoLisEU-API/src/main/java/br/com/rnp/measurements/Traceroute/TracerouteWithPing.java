 ...
 */
    public class TracerouteWithPing {
         ...
         * @param maxTtl
         */
        public void executeTraceroute(String url, int maxTtl, List<TracerouteContainer> traces) {
            this.ttl = 1;
        ...
        private class TimeOutAsyncTask extends AsyncTask<Void, Void, Void> {
            ...
            protected void onPostExecute(Void result) {
                ...
                runnableTimeout = new Runnable() {
                    ...
                    public void run() {
                        if (task != null) {
                            //Log.e("TRACEROUTE", ttlTask + " task.isFinished()" + finishedTasks + " " + (ttlTask == finishedTasks));

                            if (ttlTask == finishedTasks) {
                                //Log.e("TRACEROUTE", context.getString(R.string.timeout));
                                task.setCancelled(true);
                                task.cancel(true);
                                //context.stopProgressBar();
                                finishTrces = true;
                                for(TracerouteContainer trace: traces){
                                    System.out.println("Timeout: " + trace.getHostname() + ":" + trace.getIp());
                                }
        ...
        private class ExecutePingAsyncTask extends AsyncTask<Void, Void, String> {
                    ...
                    try {
                        if (!"".equals(result)) {
                            if (context.getString(R.string.no_connectivity).equals(result)) {
                                Log.e("TRACEROUTE", context.getString(R.string.no_connectivity));
                            } else {
                                ...
                                if (traces.get(traces.size() - 1).getIp().equals(ipToPing)) {
                                    if (ttl < maxTtl) {
                                        ...
                                        finishTrces = true;
                                        for(TracerouteContainer trace: traces){
                                            System.out.println("Finish: " + trace.getHostname() + ":" + trace.getIp());
                                        }
                                    }
                                } else {
                                    if (ttl < maxTtl) {
            ...
            private void onException(Exception e) {
                ...
                if (e instanceof IllegalArgumentException) {
                    Log.e("TRACEROUTE", context.getString(R.string.no_ping));
                } else {
                    Log.e("TRACEROUTE", context.getString(R.string.error));
                }
