	...
	)
{
	...
	while(nleft > 0){
		if((nread = read(fd, ptr, nleft)) < 0){
			if((errno == EINTR) && !*retn_on_intr){
				...
				//__android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: I2Readni return -1");
				return(-1);
			}
		} else if(nread == 0){
			//__android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: errno = %d", errno);
            //if(errno == EINPROGRESS){
            //	nleft = n;
            //	//__android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: EINPROGRESS - continue");
            //	continue;
            //return 32;
            //}
			/* EOF */
	...
	//__android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: I2Readni n - nleft");
