...
iperf_new_stream(struct iperf_test *test, int s)
{
    int i;
    struct iperf_stream *sp;
    char *template = strcat(tmp_dir, "/iperf3.XXXXXX");

    h_errno = 0;

    sp = (struct iperf_stream *) malloc(sizeof(struct iperf_stream));
    if (!sp) {
        __android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG", "struct iperf_stream");
    ...
    if (!sp->result) {
        free(sp);
        __android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG", "sp result");
        i_errno = IECREATESTREAM;
        return NULL;
    }
    ...
    if (sp->buffer_fd == -1) {
        __android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG", "Create and randomize the buffer");
        i_errno = IECREATESTREAM;
        free(sp->result);
        free(sp);
        return NULL;
    ...
    if (unlink(template) < 0) {
         __android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG", "Unlink template");
        i_errno = IECREATESTREAM;
        free(sp->result);
        free(sp);
        return NULL;
    ...
    if (ftruncate(sp->buffer_fd, test->settings->blksize) < 0) {
         __android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG", "ftruncate");
        i_errno = IECREATESTREAM;
        free(sp->result);
        free(sp);
        return NULL;
    ...
    if (sp->buffer == MAP_FAILED) {
        __android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG", "MAP_FAILED");
        i_errno = IECREATESTREAM;
        free(sp->result);
        free(sp);
        return NULL;
...
iprintf(struct iperf_test *test, const char* format, ...)
{
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG_IPRINT", "AQUIII");
    /*
    ...
    if (test->role == 'c') {
	    if (test->title) {
	    printf("%s:  ", test->title);
	    __android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG_IPRINT_C_TITLE", "%s:  ", test->title);
	    }
        ...
        va_end(argp);
        printf("%s", linebuffer);
        __android_log_print(ANDROID_LOG_DEBUG, "IPERF3_LOG_IPRINT_S", "%s", linebuffer);
