        ...
        )
{
    ...
    //I2ErrLog(eh,"Spawn Local Server");
     ...
     */
     //I2ErrLog(eh,"Now spawn the child process to be the mock-daemon");
    pid = fork();

    /* fork error */
    //I2ErrLog(eh,"fork error");
    if(pid < 0){
        I2ErrLog(eh,"fork(): %M");
        return NULL;
    }
    ...
    //__android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Now implement child \"server\"");
    //I2ErrLog(eh,"Now implement child \"server\"");
     ...
     */
      //I2ErrLog(eh,"BWLContextconfigSet(BWLAccessPriority,BWLErrOK)");
    if(!app.opt.verbose){
    ...
    if( (childwait = BWLContextConfigGetV(lctx,BWLChildWait))){
         ...
         * as well.
         */
         I2ErrLog(eh,"BWLContextConfigSet(ChildWait)");
    ...
    )
{
    ...
    I2Boolean           prev_test_failed;
     ...
     I2ErrLog(eh,"%s %s %s %s %s %s", argv[0], argv[1], argv[2], argv[3], argv[4], argv[5]);
...
negotiate_test(ipsess_t server_sess, ipsess_t client_sess, BWLTestSpec *test_options)
{
    ...
    /* Pick tool */
    if(!server_sess){
        I2ErrLog( eh, "Server_sess = NULL");
    }
    if(!server_sess->avail_tools){
            I2ErrLog( eh, "Server_sess->avail_tools = NULL");
        }

    /* Check if the requested tool is available at both servers. */
        //I2ErrLog( eh, "Available in-client: %s", BWLToolGetToolNames( ctx, client_sess->avail_tools ) );
        //I2ErrLog( eh, "Available in-server: %s", BWLToolGetToolNames( ctx, server_sess->avail_tools ) );
...
setup_results_storage(ipsess_t sess)
{
    ...
    if (app.opt.bidirectional_results ||
       ...
       if(app.opt.printfiles){
            ...
            char *str_file = "/tmp.bwctl1";
            char *str_path = (char *) malloc(1 + strlen(tmp_dir)+ strlen(str_file) );
            strcpy(str_path, tmp_dir);
            strcat(str_path, str_file);
            if(!(sess->results_fp = fopen(str_path,"w+"))){
