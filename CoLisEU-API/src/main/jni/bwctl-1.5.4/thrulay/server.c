...
acl_allow_add (const char *const str)
{
	...
	if ((rc = getaddrinfo(str, NULL, &hints, &res)) != 0) {
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Error: getaddrinfo(): failed in ACL add "
			"operation , %s\n", gai_strerror(rc));
		return -31;
	}
...
logging_log_string (int priority, const char *s)
{
	switch (log_type) {
		case LOGTYPE_SYSLOG:
			...
			break;
		case LOGTYPE_STDERR:
			__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "%s %s\n", logging_time(), s);
			fflush(stderr);
			break;
	}
}
...
{
	__android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "thrulay_server_init!!!");
	int rc;
...
thrulay_server_listen(int port, int window)
{
	...
	if ((n = getaddrinfo(NULL, service, &hints, &res)) != 0) {
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Error: getaddrinfo() failed: %s\n",
			gai_strerror(n));
		return -27;
	}
	ressave = res;
