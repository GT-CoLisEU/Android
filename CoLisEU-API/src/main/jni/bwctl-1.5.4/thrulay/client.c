	...
	void **saptr, socklen_t *lenp)
{
	...
	if ((n = getaddrinfo(server_name, service, &hints, &res)) != 0) {
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "getaddrinfo(): %s\n", gai_strerror(n));
		return -9;
	}
	ressave = res;
...
read_greeting(int s)
{
	...
	if (buf[greetlen - 1] != '+') {
		...
		buf[rc] = '\0';
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "server said: %s", buf);
		if (buf[rc-1] != '\n')
			__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "\n");
		return -15;
	}
...
#else
	srand(time(NULL)+ (unsigned int)(uintptr_t)random_state);
#endif
