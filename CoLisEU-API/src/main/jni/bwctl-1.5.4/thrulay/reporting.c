...
quantile_alg_error(int rc)
{
	switch (rc) {
	case -1:
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Error: quantiles not initialized.");
		break;
	case -2:
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Error: NEW needs an empty buffer.");
		break;
	case -3:
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Error: Bad input sequence length.");
		break;
	case -4:
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Error: Not enough buffers for COLLAPSE.");
		break;
	default:
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG", "Error: Unknown quantile_algorithm error.");
	}
	//exit(1);
}
...
main(int argc, char *argv[])
{
	...
	if (-1 == rc || -1 == rc2 || -1 == rc3) {
		__android_log_print(ANDROID_LOG_DEBUG, "JNI_ERROR_LOG","An error occurred while computing delay "
			"quantiles. %d %d %d\n",rc, rc2, rc3);
		//exit(1);
	}
