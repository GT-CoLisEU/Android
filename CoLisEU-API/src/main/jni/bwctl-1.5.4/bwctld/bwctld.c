...
#define OPTSTRING   OPTBASESTRING
    ...
    opts.confdir = conf_dir;
    opts.vardir = tmp_dir;

    if(!getcwd(opts.cwd,sizeof(opts.cwd))){
        perror("getcwd()");
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "getcwd()");
        exit(1);
    }
        ...
        switch (ch){
            case 'c':    /* -c "Config directory" */
                if (!(opts.confdir = strdup(optarg))) {
                    /* eh isn't setup yet...*/
                    perror("strdup()");
                    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "strdup()");
                    exit(1);
                }
                break;
            default:
            ...
            case 'e':    /* -e "syslog err facility" */
                ...
                if(fac == -1){
                    fprintf(stderr,
                            "Invalid -e: Syslog facility \"%s\" unknown\n",
                            optarg);
                    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI","Invalid -e: Syslog facility \"%s\" unknown\n", optarg);
                    exit(1);
                }
                syslogattr.facility = fac;
                break;
    ...
    if(! errhand) {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "%s : Couldn't init error module\n", progname);
        exit(1);
    }
    ...
    if(!(ctx = BWLContextCreate(errhand,NULL))){
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "BWLContextCreate ERRO");
        exit(1);
    }
