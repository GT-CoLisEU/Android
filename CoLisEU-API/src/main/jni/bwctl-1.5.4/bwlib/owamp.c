...
//const char *tmp_dir_owamp = tmp_dir + "/bwctl_owamp";
        ...
        )
{
    ...
    if(tsess->conf_server){
        ...
        char *tmp_dir_owamp;
        strcat(tmp_dir_owamp, tmp_dir);
        strcat(tmp_dir_owamp, "/bwctl_owamp");
        snprintf(buf, sizeof(buf), "%s/%d", tmp_dir_owamp, tsess->tool_port);

        if (do_mkdir(tmp_dir_owamp, 0700) != 0) {
            BWLError(ctx,BWLErrWARNING,BWLErrUNKNOWN,
