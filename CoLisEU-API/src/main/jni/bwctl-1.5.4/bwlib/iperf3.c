        ...
        )
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: Now run iperf3!");
        int nargs = 0;
        int nargs2 = 0;
        while(ipargs[nargs]){
            if(nargs > 0){
                if(strcmp(ipargs[nargs], "-B") && strcmp(ipargs[nargs-1], "-B") && strcmp(ipargs[nargs], "-Z") && strcmp(ipargs[nargs], "--omit") && strcmp(ipargs[nargs-1], "--omit")){
                    strcpy(ipargs2[nargs2], ipargs[nargs]);
                    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: %s", ipargs2[nargs2]);
                    nargs2++;
                }
            } else {
                strcpy(ipargs2[nargs2], ipargs[nargs]);
