       ...
       )
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "server_addr = %s", I2AddrNodeServName(server_addr, tmpstr, &tmpstrlen));
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "local_addr = %s", local_addr);
    int i;
    for(i = 0; i<14; i++){
        sprintf(sa_data, "%d", ai->ai_addr->sa_data[i]);
        strcat(sa_data_f, sa_data);
        strcat(sa_data_f, "-");
    }
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "addrinfo->ai_canonname = %s", ai->ai_canonname);
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "addrinfo->sockaddr->sa_data[] = %s", sa_data_f);
    ...
    if (errno == EINPROGRESS) {
        ...
        //ERRO OCORRE AQUI!!!
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "getsockopt");
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "fd = %d, SOL_SOCKET = %d, SO_ERROR = %d", fd, SOL_SOCKET, SO_ERROR);
        getsockopt(fd, SOL_SOCKET, SO_ERROR, &err, &err_len);
        if (err != 0){
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "erro = %d", err);
            goto cleanup;
        }
    }
        ...
        )
{
     ...
     */
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Easy case - application provided socket directly.");
     ...
     */
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Initialize addrinfo portion of server_addr record.");
     ...
     * connect.
     */
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Now that we have addresses");
    if( !BWLContextConfigGetV(cntrl->ctx,BWLIPv4Only)){
#ifdef	AF_INET6
        __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "AF_INET6");
        for(ai=fai;ai;ai=ai->ai_next){
            ...
            if( (rc = TryAddr(cntrl,ai,local_addr,server_addr)) == 0) {
                __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "return 0");
                return 0;
            }
            if(rc < 0) {
                __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "goto error");
                goto error;
            }
        }
#endif
     ...
     */
     __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "Now try IPv4 addresses.");
    if( !BWLContextConfigGetV(cntrl->ctx,BWLIPv6Only)){
        for(ai=fai;ai;ai=ai->ai_next){
            struct sockaddr_in saddr4;
            __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "for");
            if(ai->ai_family != AF_INET) continue;

            /* avoid type punning by using memcpy instead of casting ai_addr */
            memcpy(&saddr4,ai->ai_addr,sizeof(saddr4));

            __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "TRY ADDR");
            if( (rc = TryAddr(cntrl,ai,local_addr,server_addr)) == 0) {
                __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "return 0");
                return 0;
            }
        ...
        )
{
    ...
    *err_ret = BWLErrOK;
    ...
    char tmpstr[200];
    size_t tmpstrlen = sizeof(tmpstr);
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: server_addr = %s\n", I2AddrNodeServName(server_addr, tmpstr, &tmpstrlen));
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: local_addr = %s\n", local_addr);
    __android_log_print(ANDROID_LOG_DEBUG, "LOG_JNI", "bwctl: %" PRIu32 "\n", mode_req_mask);
